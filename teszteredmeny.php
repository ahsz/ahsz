...
<?php
	...
	function mydraw(){
			require "config.php";
			$result=mysqli_query($con,"SELECT ifnull(NUM_OF_TEST,'') AS NPASS, ifnull(NUM_OF_FAIL,'') AS NFAILED, ifnull(NUM_OF_INC,'') as NINC, ifnull(DATE,'') AS DDATE FROM TEST WHERE TEAM_ID=$_SESSION['TEAM_ID'] order by DATE DESC LIMIT 1");
